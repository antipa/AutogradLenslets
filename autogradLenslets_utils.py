...
def forward_sim(psf, im, padfrac=1/2, crop_size = (0, 0), noise_var = .005):
...
#     print(np.shape(cropped))
    return(cropped + noise_var * np.random.standard_normal(crop_size))
...
def admm2d_learn(y, psf_in, tau, niter, mu1, mu2, mu3, options = 'default'):
    #Inputs:
    # y : measurement
    # psf_in : point spread function
    # tau : regularizer
    ...
    alpha2k_2 = copy.deepcopy(sk[:,:-1])
    HtH = np.abs(H*H_conj)
    #Smult = 1/(mu1[0]*HtH + mu2[0]*LtL + mu3[0])
    CtC = pad(np.ones_like(y))
    Vmult = 1/(CtC + mu1[0])
    Hskp = np.zeros_like(Vmult)
    dual_resid_s = []
    ...
    for n in range(niter):
        Smult = 1/(mu1[n]*HtH + mu2[n]*LtL + mu3[n])
        Vmult = 1/(CtC + mu1[n])
        Lsk1, Lsk2 = L(sk)

        ukp_1, ukp_2 = soft_2d_gradient(Lsk1 + alpha2k_1/mu2[n], Lsk2 + alpha2k_2/mu2[n], tau[n]/mu2[n])
        #Hsk = copy.deepcopy(Hskp);   ##### DEEPCOPY!?
        Hsk = Hskp
        vkp = Vmult*(mu1[n]*(alpha1k/mu1[n] + Hsk) + Cty)
        wkp = np.maximum(alpha3k/mu3[n] + sk, 0)
        skp_numerator = mu3[n]*(wkp - alpha3k/mu3[n]) + mu2[n]*Ltv(ukp_1 - alpha2k_1/mu2[n], ukp_2 - alpha2k_2/mu2[n]) + mu1[n] * Hadj(vkp - alpha1k/mu1[n])
        skp = np.real(np.fft.ifft2(Smult * np.fft.fft2(skp_numerator)))

        Hskp = Hfor(skp)
        r_sv = Hskp - vkp
        dual_resid_s.append(mu1[n] * np.linalg.norm(Hsk - Hskp,'fro'))
        primal_resid_s.append(np.linalg.norm(r_sv,'fro'))
 ...
 #       mu1, mu1_update = update_param(mu1, resid_tol, mu_inc, mu_dec, primal_resid_s[-1], dual_resid_s[-1])
        ...
        r_su_1 = Lskp1 - ukp_1
        r_su_2 = Lskp2 - ukp_2
        dual_resid_u.append(mu2[n]*np.sqrt(np.linalg.norm(Lsk1 - Lskp1,'fro')**2 + np.linalg.norm(Lsk2 - Lskp2,'fro')**2))
        primal_resid_u.append(np.sqrt(np.linalg.norm(r_su_1,'fro')**2 + np.linalg.norm(r_su_2,'fro')**2))


        #mu2, mu2_update = update_param(mu2, resid_tol, mu_inc, mu_dec, primal_resid_u[-1], dual_resid_u[-1])
        alpha2k_1+= mu2[n]*r_su_1
        alpha2k_2+= mu2[n]*r_su_2


        r_sw = skp - wkp
        dual_resid_w.append(mu3[n]*np.linalg.norm(sk - skp,'fro'))
        primal_resid_w.append(np.linalg.norm(r_sw,'fro'))

        #mu3, mu3_update = update_param(mu3[n], resid_tol, mu_inc, mu_dec, primal_resid_w[-1], dual_resid_w[-1])
        alpha3k += mu3[n]*r_sw
        ...
        #mu_update = 0
            ...
            #mu_update = 1
#         if n < (niter-1):
#             Smult = 1/(mu1[n+1]*HtH + mu2[n+1]*LtL + mu3[n+1])
#             Vmult = 1/(CtC + mu1[n+1])
